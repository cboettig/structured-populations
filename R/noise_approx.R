...
twoD <- function(t, y, p){
	...
	yd4 <- 2*( p["b2"] * (p["K"] - y[1] - 2*y[2]) - p["d2"] - p["c2"] * y[1] ) * y[4]	+		# 2 \partial_y beta_1(x,y)  * \sigma_y^2 +
		...
		p["b2"] * y[1] * (p["K"] - y[1] - y[2]) + p["d2"] * y[1] + p["c2"] * y[1] * y[2]		# \beta_2
	# cov 
	yd5 <- (  (p["b1"] * (p["K"] - 2*y[1] - y[2]) - p["d1"] + p["c1"] * y[2] )			+		# (\partial_x \alpha_1 +
		( p["b2"] * (p["K"] - y[1] - 2*y[2]) - p["d2"] - p["c2"] * y[1] ) ) * y[5]		+		# \partial_y \beta_1 )* cov + 
