...
source("../R/plots.R")
	...
	Xo <- numeric(adults)
	Xo[1] <- 100
	beetle_data_gamma <- linear_noise_approx(Xo, times, gamma_beetle_pars, b_gamma, d_gamma, J_gamma, T_gamma, Omega=volume)
	ibm_gamma <- gamma_beetles_ibm(Xo=c(Xo[1], Xo[11], Xo[21], Xo[31]), par=beetle_pars, time = times, reps = 100)
