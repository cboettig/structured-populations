...
T<- 200
require(stochPop)
pars = c(Xo = 570, e = 0.5, a = 150, K = 1000, h = 200, 
    i = 0, Da = 0.0, Dt = 100)
sn <- saddle_node_ibm(pars, times=seq(0,T, length=200))
...
png("ou_boot",800,200); plot_bootstrap(model_boots, model=1); dev.off()
png("wa_boot",800,200); plot_bootstrap(model_boots, model=2); dev.off()
png("cpt_boot",800,200); plot_bootstrap(model_boots, model=3); dev.off()
